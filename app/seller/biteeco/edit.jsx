    ...
    try {
      ...
      if (result.success && result.wasteItems) {
        ...
        if (item) {
          ...
          showAlert(t('common.error'), t('editBiteEcoItem.alerts.itemNotFound'), [{ text: t('common.ok'), onPress: () => router.back() }], 'error');
        }
      } else {
        showAlert(t('common.error'), t('editBiteEcoItem.alerts.fetchListFailed'), [{ text: t('common.ok'), onPress: () => router.back() }], 'error');
      }
    } catch (error) {
      console.error('Error fetching item data:', error);
      showAlert(t('common.error'), t('editBiteEcoItem.alerts.fetchItemFailed'), [{ text: t('common.ok'), onPress: () => router.back() }], 'error');
    } finally {
      setInitialLoading(false);
    }
  };
  ...
  const handleUpdateItem = async () => {
    ...
    try {
      ...
      if (result.success) {
        showAlert(
          t('common.success'),
          t('editBiteEcoItem.alerts.updateSuccess'),
          [
            {
              text: t('common.ok'),
              onPress: () => router.back(),
        ...
        showAlert(t('common.error'), result.message || t('editBiteEcoItem.alerts.updateFailed'), [{ text: t('common.ok') }], 'error');
      }
    } catch (error) {
      console.error('Error updating waste item:', error);
      showAlert(t('common.error'), error.message || t('editBiteEcoItem.alerts.updateFailed'), [{ text: t('common.ok') }], 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>{t('editBiteEcoItem.header.title')}</Text>
          <View style={{ width: 26 }} />
        </View>
          ...
          <View style={styles.content}>
            ...
            <View style={[styles.introCard, styles.shadow]}>
              <View style={styles.introIconBox}>
                <MaterialIcons name="edit" size={24} color={COLORS.GREEN4} />
              </View>
              <View style={styles.headerText}>
                <Text style={styles.introTitle}>{t('editBiteEcoItem.intro.title')}</Text>
                <Text style={styles.introSubtitle}>
                  {t('editBiteEcoItem.intro.subtitle')}
                </Text>
              </View>
            </View>
            ...
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>{t('editBiteEcoItem.fields.photo.label')}</Text>
            ...
            >
              {loading ? (
                ...
                <Text style={styles.submitButtonText}>{t('editBiteEcoItem.submitButton')}</Text>
              )}
            </TouchableOpacity>
          </View>
        </ScrollView>
